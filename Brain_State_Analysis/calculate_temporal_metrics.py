...
if __name__ == "__main__":
    ...
    for thre_type in ['thr_0', 'nonthr']:
        if thre_type == 'nonthr':
            ...
            n_states = 8 # Includes NOTA initially
        ...
        if thre_type == 'thr_0':
            print("  Removing NOTA state data to maintain consistency (8 -> 7 states)...")
            # Slice to keep only the first 7 states
            MDDfo = MDDfo[:-1, :]
            MDDdt = MDDdt[:-1, :]
            MDDar = MDDar[:-1, :]
            HCfo = HCfo[:-1, :]
            ...
            # Slice TP matrices (n_subs, n_states, n_states) -> keep 7x7
